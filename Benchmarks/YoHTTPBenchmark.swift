...
actor TodoStore {
	static let incompleteID = UUID(uuidString: "00000000-0000-0000-0000-000000000001")!
	static let completeID = UUID(uuidString: "00000000-0000-0000-0000-000000000002")!

	private var todos: [UUID: Todo] = {
		let now = Date()
		let first = Todo(
			id: TodoStore.incompleteID, title: "Read the YoHTTP README",
			completed: false, tags: ["docs"], createdAt: now, updatedAt: now)
		let second = Todo(
			id: TodoStore.completeID, title: "Ship an in-memory REST API",
			completed: true, tags: ["example", "swift"], createdAt: now,
			updatedAt: now)
...
struct Health {
	func handle(_ request: consuming Request, _ response: borrowing Response) throws {
		API.json(["status": "ok"], version: request.head.version, to: response.writer())
	}
}
...
private let eventLoopCount = max(
	Int(ProcessInfo.processInfo.environment["YOHTTP_BENCHMARK_EVENT_LOOP_COUNT"] ?? "") ?? 1,
	1
)

@main
private enum YoHTTPBenchmark {
	static func main() async throws {
		...
		let server = YoHTTPServer(
			runtimeConfiguration: .init(eventLoopCount: eventLoopCount))
		server.handler(router)

		let port =
			Int(ProcessInfo.processInfo.environment["YOHTTP_BENCHMARK_PORT"] ?? "")
			?? 8081
		print(
			"yohttp benchmark server listening on 127.0.0.1:\(port); event loops: \(eventLoopCount)"
		)
		try await server.listen(
			ServerConfiguration(hostname: "127.0.0.1", port: port, serverName: nil))
	}
}
